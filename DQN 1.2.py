...
# env = gym.make('LunarLander-v2').unwrapped      # 使用gym库中的环境：LunarLander，且打开封装
...
for i in range(episode_size):                                           # 400个episode循环
    print('<<<<<<<<<Episode: %s' % i)
    s = env.reset()                                                     # 重置环境
    ...
    while True:                                                         # 开始一个episode (每一个循环代表一步)
        ...
        if done:       # 如果done为True
            ...
            R_list.append(round(episode_reward_sum, 2))
            break                                                       # 该episode结束
plt.plot(range(episode_size), R_list)
env.close()
plt.show()